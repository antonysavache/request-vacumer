...
export class MessageFilterService {
  ...
  filterMessage(messageText: string, config: MonitoringConfig): FilterResult {
    this.logger.log(`🔍 DETAILED FILTERING for: "${messageText}"`);
    
    if (!messageText || messageText.trim().length === 0) {
      this.logger.log(`❌ Empty message detected`);
      return {
        shouldForward: false,
        matchedKeywords: [],
    ...
    this.logger.log(`📝 Normalized text: "${normalizedText}"`);

    // Проверка минимальной длины сообщения
    if (config.minMessageLength && normalizedText.length < config.minMessageLength) {
      this.logger.log(`❌ Message too short: ${normalizedText.length} < ${config.minMessageLength}`);
      return {
        shouldForward: false,
        matchedKeywords: [],
    ...
    if (config.excludeKeywords && config.excludeKeywords.length > 0) {
      this.logger.log(`🚫 Checking exclude keywords: [${config.excludeKeywords.join(', ')}]`);
      
      const foundExcludeKeywords = config.excludeKeywords.filter(keyword => {
        const found = normalizedText.includes(keyword);
        this.logger.log(`  - "${keyword}": ${found ? '❌ FOUND' : '✅ not found'}`);
        return found;
      });

      if (foundExcludeKeywords.length > 0) {
        this.logger.log(`❌ Message contains exclude keywords: ${foundExcludeKeywords.join(', ')}`);
        return {
          shouldForward: false,
          matchedKeywords: [],
    ...
    this.logger.log(`🎯 Checking main keywords: [${config.keywords.join(', ')}]`);
    
    const matchedKeywords = config.keywords.filter(keyword => {
      const found = normalizedText.includes(keyword);
      this.logger.log(`  - "${keyword}": ${found ? '✅ MATCH!' : '❌ no match'}`);
      return found;
    });

    if (matchedKeywords.length > 0) {
      this.logger.log(`✅ KEYWORDS MATCHED: ${matchedKeywords.join(', ')}`);
      return {
        shouldForward: true,
        matchedKeywords,
    ...
    this.logger.log(`❌ No keywords matched in "${normalizedText}"`);
    return {
      shouldForward: false,
      matchedKeywords: [],
  ...
  logFilterResult(
    ...
    if (result.shouldForward) {
      ...
      this.logger.log(`⏭️ [${chatTitle}] SKIP: "${preview}" -> ${result.reason}`);
    }
  }
}
