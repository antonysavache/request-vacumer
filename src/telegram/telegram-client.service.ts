...
export class TelegramClientService implements OnModuleInit {
  ...
  private async connectAndAuth(): Promise<void> {
    try {
      ...
      // ВАЖНО: Получаем и выводим session string после успешной авторизации
      const sessionString = this.client.session.save() as unknown as string;
      if (sessionString) {
        // Выводим в консоль для копирования на сервер
        this.logger.log('');
        this.logger.log('='.repeat(80));
        this.logger.log('🎉 AUTHENTICATION SUCCESSFUL!');
        this.logger.log('');
        this.logger.log('📋 COPY THIS SESSION STRING FOR SERVER DEPLOYMENT:');
        this.logger.log('');
        ...
        this.logger.log('');
        this.logger.log('⚠️  IMPORTANT: Keep this session string SECRET!');
